...
**RESOLVED 2026-09-04 (DEC-102) — accept the over-grouping, no code change.** Measured rather than argued: the 3,829 cleared pairs are **30.2% of duplicate edges** (3,844 of 12,715), welding 18,560 files into 5,868 clusters where subtraction gives 13,120 files in 4,255. Effect on the real split is small — Potholes, which carries 48% of the cleared pairs, sits at **17.7% val as-is vs 15.0% subtracted**, still holding 472 val / 386 test images; every other class is within ±1.6pp of target and the global ratio is 70.1/15.0/14.9 either way. Overriding an algorithm with a hand review on the one code path that prevents train/eval contamination was judged a poor trade for 2.7pp on one class (AGENTS.md ranks correctness above dataset quality). **Cost accepted explicitly:** DEC-090's review stays inert at split time. Worth recording because the premise nearly went the other way — subtraction was briefly feared to *delete* pothole data; it does not. `assign_splits()` unions filenames so duplicates move together, and every file receives a split regardless. Grouping concentrates a class, it never removes one.
